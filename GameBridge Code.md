...
KOPPLINGAR:
 ...
 - ANROPAS AV: core/bridge_core.py
"""
...
def function_play_welcome_message(audio_instance, gui_instance, current_voice_hotkey):
    ...
    if gui_instance:
        gui_instance.append_log("SYSTEM", f"G.A.M.E. B.R.I.D.G.E. operational. Baseline voice hotkey resolved to [{current_voice_hotkey.upper()}].")
...
def function_setup_hardware_hotkeys(core_instance):
    ...
    error_count = 0  # SÄKERHETSSPÄRR: Startar räknaren innan loopen
    ...
    while core_instance.running:
        if core_instance.gui and hasattr(core_instance.gui, 'voice_mode_btn'):
            ...
            if current_mode not in ["OFF", "AV", "off", "av", None]:
                ...
                try:
                    # INTEGRATION: Normaliserar tangenten via HardwareIO för skottsäker bakgrundsdrift
                    normalized_key = core_instance.hardware.normalize_key(raw_target_key)
                    is_listen_mode = str(current_mode).upper() in ["LISTEN", "LYSSNA"]

                    if (is_listen_mode or keyboard.is_pressed(normalized_key)) and core_instance.voice and not core_instance.voice.is_recording:
                        core_instance.voice.execute_ptt_transaction(
                            ...
                            current_mode_callback=lambda: core_instance.gui.voice_mode_btn.get()
                        )
                    
                    error_count = 0  # SÄKERHETSSPÄRR: Allt gick bra, nollställ räknaren
                    
                except Exception as e:
                    error_count += 1  # Öka räknaren vid fel
                    if error_count <= 5:  # Skriv bara ut de 5 första felen i terminalen
                        print(f"[CORE-ERROR] Keyboard state scan faulted: {e}")
                    elif error_count == 6:  # Ge en sista varning om att terminalen tystas
                        print("[CORE-WARNING] Keyboard errors repeating rapidly. Silencing terminal output to protect performance.")
...
class GameBridgeCore:
    ...
    def link_gui(self, gui_instance):
        ...
        if self.gui and hasattr(self.gui, 'internet_toggle'):
            initial_state = "ON" if self.internet_ai_enabled else "OFF"
    ...
    def update_internet_capability(self, enabled: bool):
        ...
        log_msg = "Internet AI capability activated (Port 8080 route armed)." if enabled else "Internet AI capability deactivated (Offline/Local enforcement active)."
        if self.gui:
            self.gui.append_log("SYSTEM", log_msg)

    def boot_platform_loops(self):
        """Unified entry point invoking completely isolated background executions from the functional block."""
        threading.Thread(target=function_setup_hardware_hotkeys, args=(self,), daemon=True).start()
        threading.Thread(target=function_play_welcome_message, args=(self.audio, self.gui, self.current_voice_hotkey), daemon=True).start()
    ...
    def set_channels_state(self, k1: bool, k2: bool):
        ...
        if self.matrix:
            self.matrix.update_states(k1, k2, self.matrix.ai_generation_enabled, self.matrix.internet_ai_enabled)
    ...
    def on_voice_token_resolved(self, recognized_text: str):
        ...
        if self.gui:
            sender_tag = "USER (Channel 1)" if getattr(self.gui, 'system_lang', 'en') == 'en' else "ANVÄNDARE (Kanal 1)"
...
def function_on_ai_toggle(gui_instance):
    ...
    if gui_instance.ai_toggle_switch.get() == 1:
        gui_instance.append_log("SYSTEM", gui_instance.localizer.get_text("log_ai_on") if gui_instance.localizer else "AI ON")
    else:
        gui_instance.append_log("SYSTEM", gui_instance.localizer.get_text("log_ai_off") if gui_instance.localizer else "AI OFF")
...
def function_on_telemetry_toggle(gui_instance):
    ...
    if not gui_instance.core_hub or not hasattr(gui_instance.core_hub, 'telemetry_worker'):
        return
    if gui_instance.read_telemetry_switch.get() == 1:
        gui_instance.core_hub.telemetry_worker.set_loop_state(True)
        gui_instance.append_log("I/O-SIGNAL", gui_instance.localizer.get_text("log_tel_on") if gui_instance.localizer else "Telemetry ON")
    else:
        gui_instance.core_hub.telemetry_worker.set_loop_state(False)
        gui_instance.append_log("I/O-SIGNAL", gui_instance.localizer.get_text("log_tel_off") if gui_instance.localizer else "Telemetry OFF")
...
def function_on_model_change(gui_instance, selected_model):
    ...
    if gui_instance.core_hub and hasattr(gui_instance.core_hub, 'ai_client'):
        ...
        try:
            with open(gui_instance.core_hub.config_path, "w", encoding="utf-8") as f:
                json.dump(gui_instance.core_hub.global_config, f, indent=4)
        except Exception as e:
...
def function_on_adapter_change(gui_instance, selected_adapter):
    ...
    no_adapter_text = gui_instance.localizer.get_text("no_adapter") if gui_instance.localizer else "No Adapter Loaded"
    if selected_adapter == no_adapter_text:
        ...
        gui_instance.append_log("SYSTEM", f"Adapter attached: {selected_adapter}")
        if gui_instance.core_hub:
...
def function_trigger_text_input(gui_instance):
    ...
    sender_tag = "USER (Channel 1)" if gui_instance.chat_switch.get() == 1 else "USER (Raw Stream)"
    gui_instance.append_log(sender_tag, text)

    if gui_instance.core_hub:
        threading.Thread(target=gui_instance.core_hub.process_chatt_flow, args=(text,), daemon=True).start()
...
def function_handle_log_double_click(gui_instance, event):
    ...
    try:
        ...
        if "hyperlink" in tags:
            ...
            full_line = gui_instance.log_box.get(f"{line_num}.0", f"{line_num}.end").strip()
